...
### 方法1: /plugin コマンドからインストール（推奨）

Claude Codeで以下を実行：

```
/plugin install https://github.com/ToshikiImagawa/ai-sdd-workflow-ja
```
...
│   └── doc-consistency-checker.md # ドキュメント整合性チェックスキル
├── hooks/
│   ├── check-spec-exists.sh     # 仕様書存在チェックフック
│   ├── check-commit-prefix.sh   # コミットプレフィックスチェックフック
│   └── settings.example.json    # フック設定例
├── LICENSE
└── README.md
```
