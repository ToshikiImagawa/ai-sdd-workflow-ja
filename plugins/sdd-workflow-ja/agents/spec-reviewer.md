...
5. **憲章準拠**: プロジェクト憲章（`CONSTITUTION.md`）の原則に準拠しているか

## レビュー観点

### 1. PRD / 要求図 (`requirement/`)

要求図はフラット構造（`{機能名}.md`）と階層構造（`{親機能名}/index.md`, `{親機能名}/{子機能名}.md`）の両方をサポートします。
...
| **階層構造**  | 階層構造の場合、`index.md` に概要と子要求への参照があるか |
...
| CONSTITUTION ↔ docs | 🟢 準拠 / 🔴 違反 / ⬜ 憲章なし | {詳細} |
